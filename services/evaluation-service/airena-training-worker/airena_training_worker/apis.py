...
def get_task_url(task_id: int):
    return FILE_SERVICE_BASE_URL + f"/taskAsset/{task_id}/trainer/download/"
...
def start_job(job_id, celery_task_id) -> Job:
    ...
    resp = requests.get(SCHEDULER_BASE_URL + f"/api/training_jobs/{job_id}/start/",
    ...
    submission = Submission(sid=obj["agent"], task_url=get_task_url(obj["task"]),
                      submission_url=get_submission_url(obj["agent"]), task_id=int(obj["task"]))

    return Job(id=job_id, submission=submission, run_time_limit=0, 
...
def update_job_error(job_id: int, task_id: str, error: str):
    resp = requests.get(SCHEDULER_BASE_URL + f"/api/training_jobs/{job_id}/update_job_error/",
                        headers={"Authorization": f"Token {ACCESS_TOKEN}"},
                        json={
                            "task_id": task_id,
                            "error": error
