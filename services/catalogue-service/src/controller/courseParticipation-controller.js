...
export async function getCourseParticipations(req, res) {
  try {
    ...
    res
      .status(200)
      .json({
        message: "CourseParticipations retrieved",
        data: courseParticipations,
      });
  } catch (error) {
...
export async function createCourseParticipation(req, res) {
  try {
    ...
    const participation = await CourseParticipation.create(participationData);
    res
      .status(201)
      .json({ message: "CourseParticipation created", data: participation });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
}
...
export async function createCourseParticipationBulk(req, res) {
  try {
    ...
    const response = await axios.post(
      "http://user-service:8000/users/ids-from-emails/",
      { emails }
    );
